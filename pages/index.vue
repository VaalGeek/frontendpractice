<template>
    <div class="bg-[#6A4DB2] min-h-screen" style="background-image: url('/assets/hero/1.jpg');">

        <div class="px-10 py-6 ">

            <div class="flex sm:justify-center">
                <img class="w-[140px] sm:w-[200px]" src="/assets/hero/logo.png" alt="">
            </div>
            <div class="grid gap-2 lg:grid-cols-2">

                <img class="mt-6 text-center" src="/assets/hero/hero.svg" alt="">

                <div>
                    <h1 class="text-2xl sm:text-4xl font-bold text-center text-white mt-10">Build The Community Your
                        Fans
                        Will
                        Love </h1>
                    <p class="mt-6 text-white text-center text-lg sm:text-2xl">
                        Hero re-imagines the way we build communities. You have a voice, but so does your audience.
                        Create
                        connections with your users as you engage in genuine discussion.
                    </p>
                    <div class="flex flex-row justify-center  mt-6">
                        <button class=" text-white bg-[#009CFA] rounded-full px-12 py-1 text-center text-2xl font-bold"
                            href="#">Join Us</button>
                    </div>

                

                    <div class="flex  flex-row py-16 cursor-pointer justify-center sm:justify-end gap-4 sm:py-0 lg:py-28 sm:mr-1 lg:mr-10">
                        <div class="rounded-full sm:h-14 sm:w-14 h-10 w-10  text-center sm:py-3 py-1 border-white border-2">
                            <Icon class="text-white sm:text-3xl sm:font-bold  text-xl" name="fa6-brands:facebook-f"></Icon>
                        </div>
                        <div class="rounded-full h-10 w-10 sm:h-14 sm:w-14 text-center sm:py-3 py-1 border-white border-2">
                            <Icon class="text-white text-xl sm:text-3xl sm:font-bold"  name="fa6-brands:x-twitter"></Icon>
                        </div>
                        <div class="rounded-full h-10 w-10 sm:h-14 sm:w-14 text-center sm:py-3 py-1 border-white border-2">
                            <Icon class="text-white text-xl sm:text-3xl sm:font-bold" name="fa6-brands:instagram"></Icon>
                        </div>
                    </div>


                </div>


            </div>




           






        </div>
    </div>
</template>
